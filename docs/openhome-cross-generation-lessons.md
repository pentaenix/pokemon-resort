...
Resort imports Pokerus into warm JSON as `strain_or_state`, `days`, and status. Projection sends strain/days through `pre_save_review`. Mirror-return warm merge currently strips incoming `resort_catalog.pokerus`, which protects canonical state but also prevents legitimate Pokerus gained in an older game from becoming canonical. Resort needs a typed Pokerus merge policy: preserve existing non-zero state, accept incoming non-zero infection/cured evidence, never clear on absent/zero-only imports, and project the best target-compatible byte.
...
- Exact behavior to change: add diagnostic notes/log fields for projected canonical ribbon count, Pokerus source/decision, PID constraint inputs/outputs, and return merge decisions. Add failing tests before behavior changes.
...
- Exact behavior to change: add typed merge rules for Pokerus, contest stats, friendship, memories, markings, ribbons/marks, and handler data. Preserve static fields. Merge gain-only fields by union/max. Allow legitimate mutable updates when source trainer/handler identity matches.
