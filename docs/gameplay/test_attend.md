...
- Attend Pokemon discovery accepts raw `.glb` development assets and lossless `PRGLBZ01` `.glbz` compiled assets. GLBZ payloads are decompressed and SHA-256 verified in memory before entering the same GLB parser; environment and overworld map models remain ordinary `.glb` files.
