...
- Debug scene files live under `config/gameplay/pokemon_attend/debug/`: set temporary selections such as `activePokemon`, `activeFloor`, `activeSky`, the debug Pokemon spawn stub, and TEST ATTEND-only overlay buttons there. The demo currently selects `dewpider` for transparency and eye-expression validation.
...
- Environment files live under `config/gameplay/pokemon_attend/environments/`: `environments/alola_battle_maps.json` owns ripped Alola battle map catalogs, map extension pairs, additive per-map placement overrides, weather material modes, sky/light presets, contact shadow, depth of field, and background color. `mapPlacementOverrides.<map>` adds position, yaw, and uniform-scale offsets after `floorDefaults.modelPlacement` (and any optional per-floor absolute `modelPlacement`); all-zero values follow the shared Alola map placement tool.
- `floorDefaults.modelPlacement` owns shared map placement inside the environment file: `x`/`y`/`z` move the floor set, `yawDegrees` rotates the map to inspect where the Pokemon spawns, and `scale` keeps the floor and any extension GLBs on the same sizing. Floor entries under `floors` choose the base GLB and should omit `modelPlacement` unless a map needs an absolute exception. Prefer `mapPlacementOverrides` for per-map deltas so the shared placement tool keeps working.
...
- Attend Pokemon discovery accepts raw `.glb` development assets and lossless `PRGLBZ01` `.glbz` compiled assets. Alola environments also use verified GLBZ packages, but enter the dedicated environment path described in [pokemon_attend_environments.md](pokemon_attend_environments.md), which preserves PICA material, composition, state, and motion metadata.
- Attend consumes authoritative RAE PICA material classification directly when present, while retaining the decoded-alpha promotion only for older exports. Opaque ranges submit first and translucent ranges submit sequentially afterward, so bubbles such as Dewpider's composite over the body without bgfx reordering opaque legs on top. Legacy exports still receive the VCO visibility compatibility rule; authoritative exports honor explicit hidden duplicate shells while keeping authored-visible vertex-colored features such as Dialga's spikes and Bunnelby's stomach.
...
- RAE Gen 1-7/3DS exports are consumed by policy instead of per-Pokemon hacks: `extras.rae.renderClass` drives opaque/mask/blend/additive behavior, glTF `doubleSided` preserves PICA backface culling (including translucent shells), mesh-node `renderOrder` and `defaultVisible` drive draw assembly, and `eyeSheet`/`eyeExpression` apply the bind-pose eye UV repeat/offset/mirror needed for base-color eye sheets. Separate irises use the eye mesh's authored alpha coverage as their stencil aperture, so colored sclera materials are supported. These RAE eyes should not use the older Violet-style `.lym`/`emissiveTexture` compositor path.
...
- Overlay UI is scene data. `ui.weatherButton`, `ui.viewButton`, `ui.pokemonButton`, `ui.previousPokemonButton`, `ui.textureVariantButton`, `ui.formVariantButton`, and `ui.skyButton` control button anchor, size, margins, padding, corner radius, border width, font size, fill color, border color, text color, and label prefix. The shared overlay canvas owns layout and hit testing; bgfx scenes draw the visible buttons through bgfx so the overlay is not hidden by the 3D presentation layer.
...
Pokemon with separate iris/pupil meshes use a stencil-backed eye composite in the attend bgfx renderer: eye-sclera materials write the eye mask and `eye_iris` materials draw only through that mask. The separate iris layer renders only for the configured `normal_open` frame; authored angry, happy, closed, and other expression frames already contain their complete eye shapes and replace it instead of overlapping it.
