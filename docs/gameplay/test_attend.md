...
- Attend restores strong authored 3DS texture translucency when an otherwise opaque RAE material carries `nitro.textureAlpha: translucent` and its decoded texture has substantial low alpha. This covers translucent bodies and bubbles such as Dewpider/Araquanid and the Solosis line without turning weak ETC alpha noise into blending. Opaque face layers and MASK irises retain their authored order, then promoted translucent ranges composite in a final pass so bubbles remain in front of the face. Required VCO feature layers remain enabled for vertex-colored details such as Dialga's spikes and Bunnelby's stomach, but only authored-visible shell materials receive the alpha-promotion override.
