...
void testModelCatalog() {
    ...
    expect(std::none_of(models.begin(), models.end(), [](const auto& model) {
        return model.id == "aquarium" || model.id == "aquarium_c" || model.id == "aquarium_t";
    }), "retired gallery tanks must not remain placeable editor assets");
}
