> **Vendored copy:** This directory lives inside the **Pokemon Resort** repository (`pokemon-resort/tools/OpenHome`). It is maintained as in-tree tooling (WASM `pkm_rs`, `resort-bridge`, optional Tauri shell), not as a separate published Git checkout. Upstream OpenHome remains GPL-3.0; see `LICENSE` in this folder.
...
For Pokemon Resort, prefer the bridge build notes in `pokemon-resort/docs/openhome-resort-bridge.md`. To build the full desktop app from this tree, follow [INSTALL.md](./docs/INSTALL.md).
...
your Pokémon and save files. Report problems in the Pokemon Resort repository issue tracker (this tree is vendored in-repo, not the upstream OpenHome project tracker).
